...
def get_process_network_usage(start_time, show_all):
    """
    Returns a list of processes with their network usage and additional information.
    """
    process_network_usage = []

    for proc in psutil.process_iter(['pid', 'name', 'create_time', 'io_counters', 'memory_info', 'cpu_percent', 'status']):
        try:
            pinfo = proc.info
            ...
            if pinfo['io_counters'] is not None:
                ...
                memory_usage = get_size(pinfo['memory_info'].rss)  # Resident Set Size
                cpu_usage = f"{pinfo['cpu_percent']:.2f}%"  # CPU percentage
                status = pinfo['status']
                process_network_usage.append([
                    pinfo['pid'],
                    pinfo['name'],
                    time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(pinfo['create_time'])),
                    upload,
                    download,
                    memory_usage,
                    cpu_usage,
...
def main(show_all):
    ...
    while True:
        ...
        print(tabulate(process_network_usage, headers=[
            'pid', 'name', 'create_time', 'Upload', 'Download', 'Memory Usage', 'CPU Usage', 'Status', 'Upload Speed', 'Download Speed'
        ]))
