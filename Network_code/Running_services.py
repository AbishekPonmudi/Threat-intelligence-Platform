...
def run_as_admin():
    try:
        code= sys.argv[0]
        params_shellWin32 = ''.join([code] + sys.argv[1:])
        subprocess.run(['powershell','start-process','python',f'-ArgumentList "{params_shellWin32}"','-Verb','runAs'],check=True) 
    except:
        sys.exit(1)
def get_size(bytes):
    """
    Returns size of bytes in a nice in all format like using kilo , giga , mega , tera
    """
    for unit in ["","k","G","M","T"]:
        if bytes < 1024:
